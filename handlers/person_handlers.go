...
func ScanPersonFromSource(c *gin.Context) {
	...
	if provider == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "source query parameter is required (MetArt, MetartX, Playboy, PlayboyPlus, Vixen, SexArt, LifeErotic, or EternalDesire)"})
		return
	}
