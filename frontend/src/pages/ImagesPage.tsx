...
import { Heart, Search, Palette, Trash2, Shuffle, UserPlus, X, Filter } from 'lucide-react';
...
export function ImagesPage() {
  ...
  const handleColorSearch = () => {
    if (colorSearch.trim()) setActiveColorSearch(colorSearch.trim());
  };
          ...
          <div className="flex flex-col justify-end h-full bg-gradient-to-t from-black/60 to-transparent p-2">
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-1">
                ...
                <button onClick={(e) => { e.stopPropagation(); setConfirmDeleteId(img.id); }} className="p-1" title="Delete image">
                  <Trash2 size={16} className="text-white hover:text-red-400" />
                </button>
                <button onClick={(e) => { e.stopPropagation(); setLinkPersonImageId(img.id); setPersonSearch(''); }} className="p-1" title="Link to person">
              ...
              <div className="flex items-center gap-1">
                ...
                {img.width && img.height && (
                  <span className="text-[10px] text-white/70">{img.width}x{img.height}</span>
                )}
                {colors.length > 0 && (
  ...
  return (
    <>
      <PageHeader title="Images" description="Browse and manage images">
        <Button variant={favoritesOnly ? 'primary' : 'secondary'} size="sm" onClick={() => updateFilter('favorites', !favoritesOnly)}>
      ...
      {showFilters && (
        <div className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4 mb-6">
          <div className="flex flex-wrap items-end gap-4">
            ...
            <label className="flex items-center gap-2 mb-0.5 text-sm text-zinc-400 cursor-pointer">
              <input
                type="checkbox"
                checked={onDiskOnly}
          ...
          <div className="flex items-center gap-2 mt-4 pt-4 border-t border-zinc-800">
            ...
            <span className="text-xs text-zinc-500 font-medium">Color Search</span>
            <div className="flex-1 flex gap-2">
              ...
              />
              {colorSearch && (
                <div className="w-8 h-8 rounded border border-zinc-700 shrink-0" style={{ backgroundColor: colorSearch.startsWith('#') ? colorSearch : `#${colorSearch}` }} />
              )}
            </div>
            ...
            {activeColorSearch && (
              <Button variant="ghost" size="sm" onClick={clearColorSearch}>Clear</Button>
            )}
          </div>
        </div>
      )}
